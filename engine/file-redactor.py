...
# Note that all arrays from JS are passed in as comma-separated strings, must be converted
input_file_location = sys.argv[1]
output_file_location = sys.argv[2]
record_type_start_positions = [int(x) for x in tuple(sys.argv[3].split(','))]
record_values = tuple(sys.argv[4].split(','))
redact_start_positions = [int(x) for x in tuple(sys.argv[5].split(','))]
redact_lengths = [int(x) for x in tuple(sys.argv[6].split(','))]
data_types = tuple(sys.argv[7].split(','))
